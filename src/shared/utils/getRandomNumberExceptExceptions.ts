export const getRandomNumberExceptExceptions = (
  length: number,
  exceptions: number[],
): number | Error => {
  if (length <= 0) {
  ...
  const validNumbers = Array.from({ length }, (_, i) => i).filter(
    (num) => !exceptions.includes(num),
  );

  if (validNumbers.length === 0) {
    return new Error('Все числа в диапазоне являются исключениями.', {
      cause: 'ALL_ARE_EXCEPTIONS',
    });
  }
